...
- [ ] One persistent process can connect one ready X360 target, accept sequential
...
- [ ] Focused boundary tests cover complete normalized endpoint states, reject
...
- [ ] EOF and explicit disconnect attempt neutral then unplug/release, preserve
      cleanup errors including a neutral failure, and do not rely on process drop
      alone.
- [ ] An opt-in Windows-only real-driver smoke executable is implemented for
...
- [ ] The provisioning note isolates the archived signed-driver and stale-updater
