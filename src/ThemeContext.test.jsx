...
import { createAppTheme } from "./ThemeContext";
...
describe("application theme focus treatment", () => {
  it("shares a high-contrast focus ring across global and MUI controls", () => {
    ...
    const focusRing = theme.components.MuiCssBaseline.styleOverrides[":focus-visible"];
    ...
    expect(theme.components.MuiButtonBase.styleOverrides.root["&:focus-visible, &.Mui-focusVisible"])
      .toEqual(focusRing);
    expect(theme.components.MuiCssBaseline.styleOverrides["button[data-controller-focused='true']"])
      .toEqual(focusRing);
    expect(theme.components.MuiInputBase.styleOverrides.root["&.Mui-focused"].boxShadow)
  ...
  it("injects the focus ring into the rendered global styles", () => {
    ...
    const styles = Array.from(document.head.querySelectorAll("style"), (style) => style.textContent).join("\n");
