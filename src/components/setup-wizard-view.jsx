...
/** @param {{onNext: () => void}} props - Wizard actions used by the welcome step. */
...
/** @param {{message: string, handleClose: () => void}} props - Error message and dismissal callback. */
const WizardError = ({ handleClose, message }) => (
  <Alert
    severity="error"
    onClose={() => {
      handleClose();
...
/** @param {{icon: import("react").ReactNode, title: string, subtitle: string, children: import("react").ReactNode}} props - Content inside the wizard step. */
...
/** @param {SetupWizardViewProps} props - Scan and sync actions. */
const ScanActions = (props) => (
  <Box>
    {!props.scanResult && !props.scanning && props.romsDir && (
      <Button
        variant="contained"
        size="large"
    ...
    {props.rommConnected && !props.syncResult && !props.syncing && (
      <Button
        variant="outlined"
        size="large"
        ...
        sx={{
          mb: 2,
          ml: props.romsDir && !props.scanResult && !props.scanning ? 2 : 0,
        }}
      >
        Sync RomM Library
      </Button>
...
const ScanStep = (props) => {
  const { rommConnected, romsDir, scanResult, scanning, syncResult, syncing } =
    props;
  let subtitle = "No ROM folder selected. You can scan later from Settings.";
  if (romsDir) {
    subtitle = `Ready to scan ${romsDir} for game files.`;
  } else if (rommConnected) {
    subtitle = "Sync your RomM library to get started.";
  }
