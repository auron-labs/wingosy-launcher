  ...
  <>
    <Mui.Paper sx={Shared.SETTINGS_CARD_SX}>
      ...
      <Mui.Typography
        ...
        sx={{ mb: 2 }}
      >
        For a report, include the app version above, your Windows
        version, reproduction steps, expected and actual behavior, and
        relevant redacted logs. Never share credentials, user data,
        configuration, database files, ROM names, or ROM paths.
      </Mui.Typography>
      ...
      >
        See the emulator and platform combinations included in this
        preview.
      </Mui.Typography>
    </Mui.Paper>
    <Mui.Paper sx={Shared.SETTINGS_CARD_SX}>
      ...
      <Mui.Typography
        ...
        sx={{ mb: 2 }}
      >
        Switch between desktop (default) and Immersive mode — a
        large-type, controller-friendly layout aligned with the
        Wingosy look. Optional OS fullscreen is ideal for couch play.
      </Mui.Typography>
      <Mui.Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
          ...
          <Mui.FormControlLabel
            control={
              <Mui.Switch
                slotProps={{
                  input: /** @type {any} */ ({
                    "data-testid": "immersive-mode-switch",
                  }),
                }}
                ...
                onChange={async (e) => {
                  ...
                  await settings.persistDisplayFlags(next, nextFs);
          ...
          control={
            <Mui.Switch
              ...
              onChange={async (e) => {
                ...
                settings.setFullscreenEnabled(nextFs);
                await settings.persistDisplayFlags(
        ...
        <Mui.Typography variant="caption" color="text.secondary">
          Tip: <Components.KeyboardHint>F11</Components.KeyboardHint> toggles fullscreen.
          From the Immersive library, <Components.KeyboardHint>Esc</Components.KeyboardHint>{" "}
          exits to desktop.
        </Mui.Typography>
        <Mui.Box sx={{ maxWidth: 420, mt: 2 }}>
          ...
          >
            Raise this only when the stick drifts. It applies to
            Immersive directional navigation.
          </Mui.Typography>
          <Components.SettingSlider
            ...
            onChange={(_, value) => {
              settings.setControllerDeadzone(settings.normalizeGamepadDeadzone(value));
            }}
            onChangeCommitted={async (_, value) =>
              settings.persistControllerDeadzone(value)
            }
          />
          <Mui.Button
            size="small"
            ...
            color="inherit"
            onClick={async () =>
              settings.persistControllerDeadzone(settings.defaultGamepadDeadzone)
            }
          >
            Reset deadzone
          </Mui.Button>
