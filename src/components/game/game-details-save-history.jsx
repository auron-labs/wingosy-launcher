...
export const SwitchRestoreProtectionNotice = ({
...
/** @param {{operationsDisabled: boolean, onClearStatus: () => void, onCreateBackup: () => Promise<void>, onEnableSaveSync: () => Promise<void>, onListSaves: () => Promise<void>, onRequestRestore: (save: GameDetailsSave) => void, onSyncCurrentSave: () => Promise<void>, saveStatus: GameDetailsStatus|null, saveSyncEnabled: boolean, saves: GameDetailsSave[], savesLoaded: boolean, savesLoading: boolean, switchPathInfo: GameDetailsSwitchPathInfo|null, switchSyncBusy: boolean}} props Switch save history dialog body. */
const SwitchSaveHistoryContent = ({
  onClearStatus,
  onCreateBackup,
  onEnableSaveSync,
  onListSaves,
  ...
  <>
    {saveStatus?.conflict === true ? (
      <Alert severity="warning">
        <Typography variant="subtitle2">Choose a save carefully</Typography>
        <Typography variant="body2">
          Your local and cloud saves both changed. Wingosy cannot safely merge
          them or replace either copy automatically. Choose a specific cloud
          save below only if you want to replace the local one; your current
          local save will be kept first.
        </Typography>
      </Alert>
    ) : (
      <Typography color="text.secondary" variant="body2">
        Restore a saved copy when you need it. Each restore keeps your current
        local save first.
      </Typography>
    )}
    {savesLoading && (
...
export const SwitchSaveHistoryDialog = ({
  launchActive,
  onClearStatus,
  onClose,
  onCreateBackup,
