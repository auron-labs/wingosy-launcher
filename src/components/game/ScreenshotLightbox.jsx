...
const LightboxToolbar = ({ count, onClose }) => (
  <>
    ...
    >
      {count > 1 && (
        <Stack direction="row" spacing={0.75} sx={{ alignItems: "center" }}>
          <KeyboardHint sx={{ borderColor: "rgba(255,255,255,0.35)", color: "#fff" }}>
            ← / →
          </KeyboardHint>
    ...
    <IconButton
      aria-label="Close"
      onClick={onClose}
      sx={{ color: "#fff", position: "absolute", right: 12, top: 12, zIndex: 2 }}
    >
      <CloseIcon />
    </IconButton>
  </>
...
const ScreenshotLightbox = ({
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      open={open && src !== null}
      slotProps={{ paper: { sx: { backgroundImage: "none", bgcolor: "rgba(0,0,0,0.94)" } } }}
    >
      <Box sx={{ alignItems: "center", display: "flex", height: "100%", justifyContent: "center", p: 2, position: "relative", width: "100%" }}>
        <LightboxToolbar count={count} onClose={onClose} />
        <LightboxNavigation count={count} onNext={goNext} onPrevious={goPrevious} />
        <Box alt="" component="img" src={src ?? ""} sx={{ borderRadius: 1, maxHeight: "100%", maxWidth: "100%", objectFit: "contain" }} />
        {count > 1 && (
          <Typography sx={{ bottom: 24, color: "rgba(255,255,255,0.7)", left: "50%", position: "absolute", transform: "translateX(-50%)" }} variant="caption">
            {safeIndex + 1} / {count}
          </Typography>
        )}
      </Box>
